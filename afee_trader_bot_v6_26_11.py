...
async def _check_and_send_weekly_report(session: aiohttp.ClientSession, state: dict):
    ...
    trade-loading, metric, and formatting logic as the existing
    (hours-based) Signal Performance Report."""
    try:
    ...
    async with _weekly_report_lock:
        ...
        try:
            ...
            parts = build_backtest_report(trades, summary, None, period_label=period_label,
                                           report_title="AFEE TRADER — Weekly Signal Performance Report")
            ...
            for part in parts:
                sent_ids = await broadcast_signal(session, part, state)
                if not sent_ids:
...
async def _check_and_send_monthly_report(session: aiohttp.ClientSession, state: dict):
    ...
    via state["monthly_report_last_month_id"], same reused trade-loading/
    metric/formatting logic, calendar-month boundary instead of calendar
    week.
    ...
    call. Removing that gate (present in an earlier version) was required
    for restart catch-up: a bot that was offline through the 1st and comes
    back up on, say, the 5th, must still be able to send the missed
    previous month's report immediately rather than waiting for the next
    1st."""
    try:
    ...
    async with _monthly_report_lock:
        ...
        try:
            ...
            parts = build_backtest_report(trades, summary, None, period_label=period_label,
                                           report_title="AFEE TRADER — Monthly Signal Performance Report")
            for part in parts:
                sent_ids = await broadcast_signal(session, part, state)
                if not sent_ids:
