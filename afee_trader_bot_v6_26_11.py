...
RUNTIME_STATE_KEYS = {
    ...
    # AUDIT FIX (Daily/Weekly/Monthly report independence): tracks the ONE
    # Tehran day still awaiting a CONFIRMED successful Daily Report send,
    # separately from daily_report_collecting_day (which only tracks day-
    # boundary detection and always advances regardless of send success).
    # See check_daily_report_rollover()/_publish_daily_report_for_day().
    "daily_report_pending_publish_day",
...
def build_period_report(trades: list, report_title: str, period_label: str, hashtag: str) -> str:
    ...
    """
    ordered = sorted(trades, key=lambda t: t.get("opened_at") or t.get("entry_ts") or "")
    ...
    for t in ordered:
        raw_st = t.get("raw_status") or t.get("status") or t.get("outcome", "")
        opened_date = tehran_date_of_iso(t.get("opened_at") or t.get("entry_ts"))
        direction_label = "🟢 LONG" if t.get("direction") == "BUY" else "🔴 SHORT" if t.get("direction") == "SELL" else "—"
        signal_uid = t.get("signal_uid")
...
# Dedicated to the Daily Report ONLY — guards _publish_daily_report_for_day()/
# sync_cross_day_report_edits() against a concurrent daily_report_loop() (5-min
# safety net) tick racing the daily_report_midnight_scheduler() tick. Never
# shared with _weekly_report_lock/_monthly_report_lock below: the three
# report types must stay fully independent (see check_daily_report_rollover()
# docstring for why this lock existing at all matters).
_daily_report_sync_lock = asyncio.Lock()
...
async def _publish_daily_report_for_day(session: aiohttp.ClientSession, state: dict,
    ...
    AUDIT FIX (Daily/Weekly/Monthly independence): only clears
    state["daily_report_pending_publish_day"] on a CONFIRMED successful
    send (sent_ids truthy) — mirroring exactly how _check_and_send_weekly_
    report()/_check_and_send_monthly_report() only set their own *_last_*_id
    after a confirmed send. If the send fails or raises (e.g. a transient
    Telegram rate-limit hit because the Weekly/Monthly report happened to
    broadcast in the very same tick — Saturday 00:00 Tehran is exactly when
    Daily and Weekly both fire), the pending day is left untouched so
    check_daily_report_rollover()'s next call (the 5-minute safety-net loop,
    or the next midnight-scheduler tick) retries THIS SAME report — it is
    never silently and permanently dropped, and this retry can never mark or
    suppress the Weekly/Monthly report or vice versa: each report type's
    pending/sent state lives in its own dedicated key.
    """
    try:
    ...
    async with _daily_report_sync_lock:
        try:
            ...
            if sent_ids:
                ...
                # Only NOW is report_date_str confirmed delivered — clear the
                # pending marker so it's never retried again.
                if state.get("daily_report_pending_publish_day") == report_date_str:
                    state["daily_report_pending_publish_day"] = None
                save_state(state)
                log.info(f"Daily Report published for {report_date_str}.")
            else:
                log.error(f"_publish_daily_report_for_day: Telegram delivery failed for "
                          f"{report_date_str}; leaving pending so it is retried.")
        except Exception as e:
            log.error(f"_publish_daily_report_for_day: send failed for {report_date_str}: {e} "
                      f"— leaving pending so it is retried.")
...
async def check_daily_report_rollover(session: aiohttp.ClientSession, state: dict, trades: list):
    ...
    the Tehran calendar date this function is currently collecting for):
      ...
      - The instant the Tehran date rolls over (00:00 Asia/Tehran), the
        ...
        tracking immediately moves on to the new Tehran day.

    AUDIT FIX (Daily/Weekly/Monthly report independence — root cause of
    "Daily Report wasn't sent when the Weekly Report was generated"):
    `daily_report_collecting_day` ALWAYS advances the moment a day boundary
    is detected, regardless of whether the publish below actually succeeds
    — it only ever tracked "which day are we now collecting", never "did we
    confirm-send the previous one". Previously that meant a failed/rate-
    limited send (most likely exactly on a Saturday 00:00 Tehran, the one
    moment Daily and Weekly both fire together and can contend for
    Telegram's rate limit) had NO retry path: collecting_day had already
    moved on, so the failed day's report was gone for good. `state
    ["daily_report_pending_publish_day"]` now tracks the one day still
    awaiting a CONFIRMED send, completely independently of collecting_day,
    and is only ever cleared by _publish_daily_report_for_day() on success —
    so a transient failure here is retried by the next call (5-minute
    safety-net loop or next midnight tick) exactly like Weekly/Monthly
    already retry themselves. This never touches, reads, or is read by any
    weekly_report_*/monthly_report_* state — the three reports' sent-state
    are fully independent, and sending one can never mark or suppress
    ...
    if collecting_day != today_tehran:
        # The Tehran date has advanced: the day that was just being collected
        # (`collecting_day`) is now complete and awaiting its report. Move
        # tracking to the new Tehran day immediately (so this detection can
        # never re-fire for the same boundary), but do NOT clobber an
        # already-pending, not-yet-confirmed day from an earlier failed
        # attempt — that one still takes priority and must not be lost.
        ended_day = collecting_day
        state["daily_report_collecting_day"] = today_tehran
        if not state.get("daily_report_pending_publish_day"):
            state["daily_report_pending_publish_day"] = ended_day
        save_state(state)

    pending_day = state.get("daily_report_pending_publish_day")
    if pending_day:
        await _publish_daily_report_for_day(session, state, trades, pending_day)
...
# ROOT-CAUSE FIX (Replay vs Live Scanner rate-limit collision): the pause
# gate above is REACTIVE — it only engages after Binance has already
# returned 429/418. Nothing previously capped how fast requests actually
# went OUT, which was fine for live scanning (~99 requests spread across
# ~14s via PARALLEL_WORKERS) but not for Historical Replay: each replay
# worker (_prefetch_replay_cache, up to REPLAY_PARALLEL_SYMBOLS symbols at
# once) pages a 30-day window through _fetch_historical_window in a tight
# loop with NO pacing between successive /klines calls for the same symbol
# — dozens of back-to-back requests per symbol, several symbols at once.
# That burst alone was enough to exceed Binance's real per-IP weight limit
# well before any 429 came back, and because the pause gate above is (by
# design, correctly) shared by every Binance caller in the process, tripping
# it also stalled the live scanner's own requests.
#
# Fix: a small, shared, PROACTIVE request-rate cap inside binance_request()
# itself — the one existing choke point already used by both Live and
# Replay — so total outbound Binance traffic (from either source, combined)
# never exceeds a safe rate. This prevents the 429/418 from happening in
# the first place instead of just reacting to it faster/slower, requires no
# new sleeps beyond what's needed to hold that rate, and leaves the
# existing 429/418 handling above completely intact as a fallback.
# WEIGHT-AWARE FIX (follow-up audit): a flat "N requests/sec" cap does not
# reflect Binance's actual limiting unit, which is REQUEST WEIGHT, not
# request count — Binance's fapi IP limit is 2400 weight per rolling
# 1-minute window (the same window it reports back via the
# X-MBX-USED-WEIGHT-1M response header). This bot's own endpoints are far
# from uniform: /klines with limit=1000 (used throughout Replay's
# pagination) costs weight 10 per call, while a live get_candles() call
# with a small limit costs 1-2, and /ticker/24hr called with no symbol
# (get_top_symbols(), universe selection) costs 40 by itself. A flat
# 10 req/s cap lets 10 klines(limit=1000) calls go out in one second —
# 100 weight/sec, i.e. 6000/min — nearly 2.5x Binance's real limit, so the
# previous fix was insufficient for exactly the Replay pagination pattern
# it was meant to fix.
#
# This tracks weight, not count, using the SAME shared choke point
# (binance_request()) and PREFERS Binance's own reported used-weight
# (X-MBX-USED-WEIGHT-1M header, parsed in binance_request() below) when a
# recent reading is available, since that's ground truth from Binance
# itself and automatically accounts for anything this bot doesn't know to
# weight correctly. When no recent header reading exists yet (first calls,
# or a run of failed/network-error requests with no header to read), it
# falls back to a conservative static per-endpoint weight table so pacing
# never silently reverts to "uncapped" — err on the side of overestimating
# weight for unrecognized endpoints/params.
BINANCE_WEIGHT_LIMIT_PER_MIN = 2400          # Binance's documented fapi IP weight limit per rolling minute
BINANCE_SAFE_WEIGHT_PER_MIN  = 1440          # keep usage at/under 60% of the real limit — headroom for
                                              # reporting lag, other weight consumers, and estimate error
_binance_weight_events: deque = deque()      # (timestamp, weight) — this process's own rolling estimate
_binance_rate_lock = asyncio.Lock()
_binance_reported_weight: tuple = (0, 0.0)   # (X-MBX-USED-WEIGHT-1M value, time.time() it was read)
_BINANCE_REPORTED_WEIGHT_MAX_AGE_S = 5.0     # a header reading older than this is stale; fall back to our own estimate

def _binance_estimate_weight(url: str, params: Optional[dict]) -> int:
    """Conservative static weight table for this bot's own known endpoints
    (per Binance USDⓈ-M Futures API docs), used only as a fallback when no
    fresh X-MBX-USED-WEIGHT-1M reading is available yet. Unknown
    endpoints/params default to a deliberately high weight (10) rather than
    the cheapest case, so an unrecognized call paces conservatively instead
    of slipping through uncounted."""
    params = params or {}
    ...
    if url.endswith("/ticker/bookTicker") or url.endswith("/ticker/price"):
    ...
    if url.endswith("/openInterestHist"):
        return 1
    return 10  # unrecognized endpoint — assume the heaviest case we know about

def _record_binance_reported_weight(headers) -> None:
    """Called from binance_request() after every response that has the
    header, regardless of status code — this is Binance telling us exactly
    how much of the current 1-minute window is used, which is more accurate
    than anything we can estimate locally."""
    global _binance_reported_weight
    raw = headers.get("X-MBX-USED-WEIGHT-1M") if headers else None
    if raw is None:
        return
    try:
        _binance_reported_weight = (int(raw), time.time())
    except (TypeError, ValueError):
        pass

async def _binance_pace_request(url: str, params: Optional[dict]) -> None:
    """Waits, if needed, so this request keeps total Binance weight usage
    (this process's best knowledge of it) at or below
    BINANCE_SAFE_WEIGHT_PER_MIN. Applies identically to every caller (live
    scan and replay alike) since it lives inside the single
    binance_request() wrapper both use."""
    weight = _binance_estimate_weight(url, params)
    async with _binance_rate_lock:
        while True:
            now = time.time()
            while _binance_weight_events and now - _binance_weight_events[0][0] >= 60.0:
                _binance_weight_events.popleft()
            local_used = sum(w for _, w in _binance_weight_events)

            reported_value, reported_at = _binance_reported_weight
            if now - reported_at <= _BINANCE_REPORTED_WEIGHT_MAX_AGE_S:
                # Ground truth from Binance itself — never trust an estimate
                # lower than what Binance just told us was actually used.
                current_used = max(reported_value, local_used)
            else:
                current_used = local_used

            if current_used + weight <= BINANCE_SAFE_WEIGHT_PER_MIN:
                break
            if _binance_weight_events:
                wait_s = 60.0 - (now - _binance_weight_events[0][0])
            else:
                # Reported weight alone is already at/near the cap with no
                # local events to expire (e.g. right after process start) —
                # a short fixed wait avoids a tight busy-loop while the
                # real 1-minute window rolls forward.
                wait_s = 1.0
            await asyncio.sleep(max(wait_s, 0.05))
        _binance_weight_events.append((time.time(), weight))
...
async def binance_request(session: aiohttp.ClientSession, url: str, params: dict = None,
    ...
    await _binance_pace_request(url, params)
    try:
        async with session.get(url, params=params, proxy=PROXY,
                                timeout=aiohttp.ClientTimeout(total=timeout)) as r:
            _record_binance_reported_weight(r.headers)
...
# AUDIT FIX (BTC Trend filter / Historical Candle Simulation): live's
# is_btc_trend_aligned() always calls get_candles(session, "BTCUSDT", tf, 250)
# regardless of which symbol is being scanned. During replay, get_candles()
# is shimmed to read from the CURRENT symbol's own HistoricalCandleCache
# (see _replay_ctx/get_candles_as_of) — a cache that _prefetch_replay_cache()
# only ever populated for the symbol actually being replayed (e.g. ETHUSDT),
# never for "BTCUSDT" itself unless BTCUSDT happened to be the symbol under
# replay. For every other symbol, the BTCUSDT lookup silently missed the
# cache and returned [] -> calc_ema200_slope([]) returns "FLAT" (its
# documented behavior on insufficient data) -> is_btc_trend_aligned() then
# takes the neutral "slope == FLAT" path and returns True unconditionally.
# Net effect: whenever a strategy had btc_trend_filter_enabled=True, the BTC
# Trend filter was silently a no-op in replay for the entire symbol universe
# except BTCUSDT — it could never reject a signal, no matter what BTC's real
# historical trend was, which is the opposite of a look-ahead bug (it's a
# missing-data / leakage-into-neutral bug) but produces the exact same kind
# of Live/Replay divergence this filter must never have. 260 candles gives
# the same headroom live's own 250-candle request has for calc_ema200_slope's
# 200-period EMA + 5-candle lookback, comfortably inside HistoricalCandleCache's
# pointer-based "as of" trimming (see get_candles_as_of).
BTC_TREND_LOOKBACK_CANDLES = 260
...
async def _prefetch_replay_cache(session, symbol: str, period_start_ms: int,
                                  period_end_ms: int, backtest_id: str = None,
                                  btc_trend_timeframes: Optional[set] = None) -> HistoricalCandleCache:
    """
    ...
    AUDIT FIX (BTC Trend filter): `btc_trend_timeframes`, when given, is the
    set of btc_trend_timeframe values actually configured+enabled across
    this job's active strategies (computed once by the caller — see
    _replay_symbol). For each one, this ALSO loads real "BTCUSDT" candles
    into the cache — using BTC_TREND_LOOKBACK_CANDLES lookback, matching
    live's own get_candles(session, "BTCUSDT", tf, 250) — regardless of
    what `symbol` is being replayed here. Without this, is_btc_trend_aligned()
    would look up ("BTCUSDT", tf) in a cache that only ever held `symbol`'s
    own series and silently fall back to a neutral/always-pass result (see
    the fix note above BTC_TREND_LOOKBACK_CANDLES). No look-ahead risk: this
    fetches strictly through `period_end_ms`, the same historical-window
    fetcher (with the same closed-candle guarantees) used for every other
    interval here, never anything from "now".
    """
    ...
    if btc_trend_timeframes:
        for interval in btc_trend_timeframes:
            step_ms = INTERVAL_MS.get(interval)
            if step_ms is None:
                log.warning(f"Replay BTC Trend prefetch: unknown interval '{interval}', skipping")
                continue
            fetch_start = period_start_ms - BTC_TREND_LOOKBACK_CANDLES * step_ms
            btc_candles = await _fetch_historical_window(session, "BTCUSDT", interval, fetch_start, period_end_ms)
            cache.load("BTCUSDT", interval, btc_candles)
            if DIAG_ENABLED and backtest_id:
                _diag_record_candle_hash(backtest_id, "BTCUSDT", interval, btc_candles)
    return cache
...
def _resolve_replay_outcome(cache: HistoricalCandleCache, symbol: str, trade_id: int,
                             entry: float, stop: float, tp1: float, tp2: float,
                             direction: str, signal_time_ms: int) -> int:
    """
    ...
    AUDIT FIX (Historical Candle Simulation, ENTERED-at-expiry mark-to-
    market): previously, a trade that got ENTERED but never reached TP1
    before the 24h expiry window closed was reported as "OPEN" with
    pnl_percent/rr_multiple left as None — silently scored as 0 in every
    aggregate (win rate, total PnL, total R). Live's own update_trade_
    outcomes() does NOT do this: an ENTERED trade that times out at 24h is
    mark-to-market closed at the last available close price (see the
    "elif local['status'] == 'ENTERED':" branch there). This was a real,
    measurable mismatch between what Historical Candle Simulation reported
    and what the live bot would actually have recorded for the same trade.
    Fixed below by applying the exact same mark-to-market formula, using
    the last historical close seen before expiry. A trade that already
    reached TP1_HIT (partial profit banked) but not TP2/SL by expiry is
    intentionally left alone here, exactly as live leaves it alone at 24h
    (see update_trade_outcomes()'s TP1_HIT branch, which explicitly does
    NOT force-close it) — only the true PENDING/ENTERED cases differ from
    "just report OPEN with nothing".

    Returns the ms timestamp at which this position stops being "open" for
    duplicate/cooldown purposes: the real close time if the trade actually
    resolved (TERMINAL status, or the ENTERED mark-to-market above), else
    `expiry_ms` (the trade is still open/unresolved at the end of the
    window, exactly as a live PENDING/ENTERED/TP1_HIT row would still show
    as open in `get_open_trade_keys()` until it resolves or expires).
    """
    ...
    last_close = None
    last_close_ms = None
    closed_ms = None  # ms timestamp this position actually stopped being "open" (see return value)
    for c in m1:
    ...
    # ── AUDIT FIX: mark-to-market an ENTERED (pre-TP1) trade that ran out
    # the clock without resolving — same formula, same trigger condition
    # (24h elapsed, still ENTERED) as update_trade_outcomes()'s live branch.
    if local["status"] == "ENTERED" and last_close is not None:
        mtm_pnl = ((last_close - entry) / entry * 100) if direction == "BUY" \
                  else ((entry - last_close) / entry * 100)
        local["pnl_percent"] = round(mtm_pnl, 2)
        local["rr_multiple"] = round((mtm_pnl / risk_pct) if risk_pct > 0 else 0, 3)
        local["close_price"] = last_close
        local["closed_at"] = last_candle_iso
        closed_ms = last_close_ms
    ...
    # AUDIT FIX (see _replay_symbol's duplicate/cooldown guard): tell the
    # caller exactly when this symbol+strategy position actually stops
    # being "open", so a brand-new candidate for the same symbol+strategy
    # isn't allowed to fire while the position this function just resolved
    # would still show up in live's get_open_trade_keys() (status in
    # OPEN/PENDING/ENTERED/TP1_HIT). If the position genuinely resolved
    # (TERMINAL status, or the ENTERED mark-to-market above), that's the
    # real close time; otherwise it's still open at the end of the replay
    # window, so it stays "open" through `expiry_ms`, same as it would in
    # live's DB-backed open-trade check.
    return closed_ms if closed_ms is not None else expiry_ms
...
async def _replay_symbol(session, symbol: str, period_start_ms: int, period_end_ms: int,
    ...
    # AUDIT FIX (BTC Trend filter): figure out, from this job's frozen
    # `state`, which btc_trend_timeframe value(s) are actually in play for
    # the strategies that will run this symbol — same cfg lookup
    # run_live_filters() itself uses (get_strategy_settings). Only strategies
    # with btc_trend_filter_enabled=True contribute a timeframe; if none do,
    # this stays empty and _prefetch_replay_cache() does no extra BTC fetch
    # at all (matches live: is_btc_trend_aligned() is never even called when
    # the filter is off).
    btc_trend_timeframes = {
        get_strategy_settings(state, strat_name).get("btc_trend_timeframe", "4h")
        for strat_name, _ in STRATEGIES
        if strat_name not in disabled
        and get_strategy_settings(state, strat_name).get("btc_trend_filter_enabled", False)
    }

    cache = await _prefetch_replay_cache(session, symbol, period_start_ms, period_end_ms,
                                          backtest_id, btc_trend_timeframes)
    m1 = cache.series.get((symbol, "1m"), [])
    if not m1:
        return 0, 0
    ...
    # f"{strategy}" -> sim_time_ms this symbol+strategy's position stays "open" until.
    # AUDIT FIX: this used to be a `set` that was add()-ed then immediately
    # discard()-ed around the (synchronous, fully-resolving) call to
    # _resolve_replay_outcome() below — so by the time the walk-forward loop
    # reached its NEXT 1m candle, the strategy was never actually recorded as
    # "open" anymore. That made this guard a permanent no-op: it could never
    # block a duplicate signal for the same symbol+strategy, unlike live's
    # get_open_trade_keys() check, which blocks for as long as the real DB
    # row stays in OPEN/PENDING/ENTERED/TP1_HIT (i.e. until it actually
    # resolves or hits the 24h expiry) regardless of the 1h signal-cooldown
    # window. Now stores WHEN each position stops being open, using the
    # return value of _resolve_replay_outcome(), and is checked against the
    # walk-forward loop's current simulated time `ct`.
    _open_until: dict[str, int] = {}
        ...
        for c in m1:
            ...
            for strat_name, strat_fn in STRATEGIES:
                ...
                if last_ms is not None and (ct - last_ms) < SIGNAL_COOLDOWN_WINDOW * 1000:
                    ...
                    continue
                if ct < _open_until.get(strat_name, -1):
                ...
                closed_or_expiry_ms = _resolve_replay_outcome(
                    cache, symbol, trade_id, result["entry"], result["stop"],
                    result["tp1"], result["tp2"], result["direction"], ct)
                _open_until[strat_name] = closed_or_expiry_ms
                trades_resolved += 1
    finally:
...
def load_state() -> dict:
    default = {
        ...
        "daily_report_pending_publish_day": None,
